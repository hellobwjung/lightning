...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'resnet_9blocks':
        ...
        net = BWUnet(input_nc, output_nc, ngf, norm=norm, use_dropout=use_dropout)
...
class Dec_block(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        self.prelut = nn.LeakyReLU()
        self.preluc = nn.LeakyReLU()
...
class BWUnet2(nn.Module):
    ...
    def forward(self, x):
        ...
        x01, x02, x03 = self.enc_block0(x)   # (    3, 128, 128) -> (   16, 128, 128)
        x11, x12, x13 = self.enc_block1(x03) # (   16, 128, 128) -> (  32,  64,  64)
        x21, x22, x23 = self.enc_block2(x13) # (  32,  64,  64) -> (  64,  32,  32)
        x31, x32, x33 = self.enc_block3(x23) # (  64,  32,  32) -> (  128,  16,  16)
        x41, x42, x43 = self.enc_block4(x33) # (  128,  16,  16) -> ( 256,   8,   8)
        x51, x52, x53 = self.enc_block5(x43) # ( 256,   8,   8) -> ( 256,   8,   8)
        x61, x62, x63 = self.enc_block6(x53) # ( 256,   8,   8) -> ( 256,   8,   8)
        x71, x72, x73 = self.enc_block7(x63) # ( 256,   8,   8) -> ( 256,   8,   8)
...
class BWUnet(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=16, norm='batch', use_dropout=False):
        ...
        self.enc_block0 = Enc_block(in_channels=input_nc, out_channels=ch1, pooling=None,  norm=None)
        self.enc_block1 = Enc_block(in_channels=ch1,      out_channels=ch2, pooling='max', norm=None)
        self.enc_block2 = Enc_block(in_channels=ch2,      out_channels=ch3, pooling='max', norm=None)
        self.enc_block3 = Enc_block(in_channels=ch3,      out_channels=ch4, pooling='max', norm=None)
        self.enc_block4 = Enc_block(in_channels=ch4,      out_channels=ch5, pooling='max', norm=None)
        self.enc_block5 = Enc_block(in_channels=ch5,      out_channels=ch5, pooling=None,  norm=None)
        self.enc_block6 = Enc_block(in_channels=ch6,      out_channels=ch6, pooling=None,  norm=None)
        self.enc_block7 = Enc_block(in_channels=ch7,      out_channels=ch7, pooling=None,  norm=None)
        ...
        self.dec_block2 = Dec_block(in_channels=ch3, out_channels=ch2)
        self.dec_block1 = Dec_block(in_channels=ch2, out_channels=ch1)
        self.dec_block0 = Enc_block(in_channels=ch1, out_channels=ch1//2, pooling=None,  norm=None)
        self.last       = nn.Conv2d(in_channels=ch1//2, out_channels=output_nc,
                                    kernel_size=kernel_size, stride=1, padding=(kernel_size)//2)
        self.tanh = nn.Tanh()
    ...
    def forward(self, x):
        ...
        x01, _, x03 = self.enc_block0(x)   # (    3, 128, 128) -> (   64, 128, 128)
        x11, _, x13 = self.enc_block1(x03) # (   64, 128, 128) -> (  128,  64,  64)
        x21, _, x23 = self.enc_block2(x13) # (  128,  64,  64) -> (  256,  32,  32)
        x31, _, x33 = self.enc_block3(x23) # (  256,  32,  32) -> (  512,  16,  16)
        x41, _, x43 = self.enc_block4(x33) # (  512,  16,  16) -> ( 1024,   8,   8)
        _, _,   x53 = self.enc_block5(x43) # ( 1024,   8,   8) -> ( 1024,   8,   8)
        _, _,   x63 = self.enc_block6(x53) # ( 1024,   8,   8) -> ( 1024,   8,   8)
        _, _,   x73 = self.enc_block7(x63) # ( 1024,   8,   8) -> ( 1024,   8,   8)
        ...
        _, _, d43 = self.dec_block4(x73, x41) # ( 1024,   8,   8) -> (  512,  16,  16)
        _, _, d33 = self.dec_block3(d43, x31) # (  512,  16,  16) -> (  256,  32,  32)
        _, _, d23 = self.dec_block2(d33, x21) # (  256,  32,  32) -> (  128,  64,  64)
        _, _, d13 = self.dec_block1(d23, x11) # (  128,  64,  64) -> (   64, 128, 128)
        _, _, d03 = self.dec_block0(d13 + x01) # (   64, 128, 128) -> (   32, 128, 128)
        # print()
        # print(f'<==========> d42.shape{d42.shape} <==========> d43.shape{d43.shape}')
        # print(f'<==========> d32.shape{d32.shape} <==========> d33.shape{d33.shape}')
        # print(f'<==========> d22.shape{d22.shape} <==========> d23.shape{d23.shape}')
...
def test_bwunet():
    model = BWUnet(input_nc=3, output_nc=3, ngf=16, norm='batch', use_dropout=True)
    # print(model)
...
def test_bwunet2():
    model = BWUnet2(input_nc=3, output_nc=3, ngf=16, norm='batch', use_dropout=True)
    # print(model)
    torchsummary.summary(model, input_size=(3, 128, 128))
    ## save onnx
    dummy_input = torch.randn(1, 3, 128, 128, device='cpu', requires_grad=False)
